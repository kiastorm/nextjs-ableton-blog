...
function setAnimationTime(mixer, animation, progress) {
...
function startAnimationLoop(mixer, animation, setPlaying, setProgress) {
  let lastTime = 0;
...
function useCreateAnimationMixer(model) {
  const clips =
    (model && (model.animations || (model.geometry || {}).animations)) || [];
